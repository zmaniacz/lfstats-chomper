    ...
    await tdfPool.connect(async (connection) => {
      await connection.transaction(async (client) => {
        ...
        let centerRecord = await client.maybeOne(sql`
          SELECT *
          FROM center
          WHERE region_code = ${game.metadata.regionCode} AND site_code = ${game.metadata.siteCode}
        `);

        if (!centerRecord) {
          let centerName = `Unknown ${game.metadata.regionCode}-${game.metadata.siteCode}`;
          centerRecord = await client.one(sql`
            INSERT INTO center 
              (name,short_name,region_code,site_code)
            VALUES
              (${centerName},'unk',${game.metadata.regionCode},${game.metadata.siteCode})
            RETURNING *
          `);
        }
        ...
        let playerRecords = await client.any(sql`
          INSERT INTO player (ipl_id, alias)
          VALUES (
            ${sql.join(
              [...entities]
                .filter((p) => p[1].type === "player")
          ...
          ON CONFLICT (ipl_id) DO UPDATE alias = EXCLUDED.alias
          RETURNING *
        `);
        ...
        //on to the teams
        let gameTeams = await client.many(sql`
